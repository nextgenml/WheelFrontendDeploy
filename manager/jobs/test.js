...
// contract address
const contractAddress = "0xcf0C122c6b73ff809C693DB761e7BaeBe62b6a2E";
...
const contractABI = [
  ...
  },
  {
    ...
    name: "taxHandler",
    outputs: [
      { internalType: "contract ITaxHandler", name: "", type: "address" },
    ],
    stateMutability: "view",
    type: "function",
  },
...
const balance = contract.methods.balanceOf(
  "0x9Afa066884cE723200438B672C0B8D5769E03A6A"
);
// console.log("balance", balance);
balance.call().then((balance) => {
  console.log("balance--------------", `${balance}`);
});
const maxSupply = contract.methods.totalSupply();
maxSupply.call().then((max) => {
  console.log("max--------------", `${max}`);
});

const delegates = contract.methods.delegates(contractAddress);

delegates.call().then((max) => {
  console.log("delegate--------------", `${max}`);
});
...
//   .call()
//   .then((holders) => {
//     // for each holder, get their balance
