...
const BlogForm = () => {
  ...
  async function get_user_data(offset) {
    const url = `${config.API_ENDPOINT}/get-blog-data?searchWalletAdd=${reset || WalletAdd == undefined ? "" : WalletAdd}&offset=${offset}`;
    let response = await fetch(url, {
      headers: {
        "accept": "*/*",
        "accept-language": "en-US,en;q=0.9",
    ...
    setpageCount((result.totalResult ? result.totalResult : 0) < pageSize ? 1 : Math.ceil(result.totalResult / pageSize))
  }
  ...
  useEffect(() => {
    ...
    if (prompts.length === 0) {
      get_gpt_data('List 10 ways in which social media will be improved by blockchain')
    }
  ...
  return (
    <div>
      {prompts == undefined ? <div className="d-flex justify-content-center">
        Loading...
      </div> :
        ...
        <><div className="p-2 col-md-12 col-lg-12">
          ...
          <form>
            ...
            <div className="col-md-4 offset-md-4 col-lg-4 offset-lg-4">
              <input type="text" className="form-control" id="search" placeholder="search wallet" value={WalletAdd} onChange={(e) => setWalletAdd(e.target.value)} />
            </div>
            <div className="col-md-4 offset-md-4 col-lg-4 offset-lg-4 mb-2 mt-2">
              <button type="button" disabled={isSubmit} style={{ marginRight: 5 }} className="btn btn-primary"
                onClick={onSubmit}
              >Submit</button>
              <input type="reset" className="btn btn-primary" disabled={isSubmit}
                onClick={() => { setWalletAdd(''); resetData(); }} value="Reset" />
            </div>
                      ...
                      <tr key={index}>
                        ...
                        <td className="text-center">
                          <div className="form-check form-switch">
                            <input className="form-check-input" type="checkbox" role="switch" checked={user.validated_flag} onChange={() => updateData("vf", user, index)} />
                          </div>
                        </td>
                        ...
                        <td className="text-center">
                          ...
                          <div className="form-check form-switch">
                            <input className="form-check-input" type="checkbox" role="switch" checked={user.paid_flag} onChange={() => updateData("pf", user, index)} />
                          </div>
                        </td>
                      </tr>
                    );
